/*! JRoll-Infinite v2.1.0 ~ (c) 2015-2016 Author:BarZu Git:https://git.oschina.net/chenjianlong/JRoll2/ */
(function(window, document, JRoll) {
	...
	JRoll.prototype.infinite = function(params) {
		...
		function callback(data) {
			...
			lock = false;
			//已加载全部数据
			//如果data的length为0，或者data的length小于size，则认定加载完成
			if(!data||data.length==0) {
				me.scroller.innerHTML = me.scroller.innerHTML.replace(clearRegExp, "") + options.completeTip;
				me.refresh();
				me.options.total = me.options.page;
				return;
			};
			...
			setTimeout(function() {
				...
				if(data.length<me.options.size) {
					me.scroller.innerHTML = me.scroller.innerHTML.replace(clearRegExp, "") + options.completeTip;
					me.refresh();
					me.options.total = me.options.page;
				};
	...
	JRoll.prototype.setTotal = function(_total) {
		this.options.total = _total
	}
